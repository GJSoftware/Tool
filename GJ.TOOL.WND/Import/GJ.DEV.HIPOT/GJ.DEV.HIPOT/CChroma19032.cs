...
namespace GJ.DEV.HIPOT
{
    public class CChroma19032 : IHP
    {
                ...
                switch (stepList[1])
                {
                    case "AC":
                        ...
                        for (int i = 0; i < stepList.Length - 5; i++)
                            stepVal.Add(System.Convert.ToDouble(stepList[i + 2]));
                        ...
                        stepVal[5] = stepVal[4];//TEST TIME                        
                        stepVal[6] = stepVal[5];//FALL TIME 
                        break;
                    case "DC":
                        ...
                        stepVal[2] = stepVal[2] * 1000;//LOW LIMIT
                        stepVal[3] = stepVal[3] * 1000;//ARC LIMIT
                        stepVal[5] = stepVal[4];//DWELL TIME                        
                        stepVal[6] = stepVal[5];//TEST TIME 
                        stepVal[7] = stepVal[6]; //FALL TIME
                        break;
                    case "IR":
                    ...
                    case "OSC":
                        ...
                        break;
                    case "GB":
                        stepName = EStepName.GB;
                        for (int i = 0; i < stepList.Length - 7; i++)
                            stepVal.Add(System.Convert.ToDouble(stepList[i + 2]));
                        if (stepVal.Count != CHPPara.C_GBItem.Length)
                        {
                            er = "获取步骤数据错误:" + rData;
                            return false;
                        }
                        stepVal[0] = stepVal[0] / 1000;//CURRENT
                        stepVal[1] = stepVal[1] / 1000000;//LOW LIMIT
                        stepVal[2] = stepVal[2] / 1000000;//HIGHT LIMIT
                        stepVal[3] = stepVal[3];//TEST TIME                    
                        stepVal[4] = stepVal[4];//TWIN PORT TIME 
                        break;
                    default:
            ...
            try
            {
                ...
                for (int i = 0; i < step.Count; i++)
                {
                    ...
                    switch (step[i].name)
                    {
                        ...
                        case EStepName.GB:
                            //CURRENT
                            cmd = "SAFE:STEP" + stepNo + ":GB " + ((int)(step[i].para[0].setVal * 1000)).ToString();
                            if (!writeCmd(cmd, out er))
                                return false;
                            //LOW LIMIT
                            cmd = "SAFE:STEP" + stepNo + ":GB:LIM:LOW " + ((int)(step[i].para[1].setVal * 1000000)).ToString();
                            if (!writeCmd(cmd, out er))
                                return false;
                            //HIGHT LIMIT
                            cmd = "SAFE:STEP" + stepNo + ":GB:LIM:HIGH " + ((int)(step[i].para[2].setVal * 1000000)).ToString();
                            if (!writeCmd(cmd, out er))
                                return false;
                            //TEST TIME   
                            cmd = "SAFE:STEP" + stepNo + ":GB:TIME: " + step[i].para[3].setVal.ToString();
                            if (!writeCmd(cmd, out er))
                                return false;
                            //TWIN PORT
                            string onOff = "On";
                            if (step[i].para[4].setVal == 0)
                                onOff = "OFF";
                            cmd = "SAFE:STEP" + stepNo + ":GB:TPOR " + onOff;
                            if (!writeCmd(cmd, out er))
                                return false;
                            break;
                        case EStepName.PA:
        ...
        {
            return writeCmd("SAFE:STAR", out er);
        }
        /// <summary>
        /// 停止测试
        /// </summary>
        ...
        {
            return writeCmd("SAFE:STOP", out er);
        }
        /// <summary>
        /// 读取测试状态
        /// </summary>
                    ...
                    for (int i = 0; i < stepMax; i++)
                    {
                        ...
                        if (C_HPCode.ContainsKey(code))
                        {
                            if (C_HPCode[code] == "PASS")
                            {
                                ...
                                uut[uutNo].Step[i].Code = string.Empty;
                            }
                            else
                            {
        ...
        private bool saveProgram(string proName, out string er)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (!sendCmdToHP(cmd, out rData, out er))
                        return false;
                    string[] rStep = rData.Split(',');
                    int proIndex = System.Convert.ToInt16(rStep[0]);
