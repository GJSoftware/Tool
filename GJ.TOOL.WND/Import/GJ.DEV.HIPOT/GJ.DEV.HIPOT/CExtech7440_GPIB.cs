...
{
    public class CExtech7440_GPIB : IHP
    {
        #region 通信指令
        ...
        /// 设定缓升时间(0,0.5-999.9)
        /// </summary>
        private const string T_delay_time_ir = "SW";
        /// <summary>
        /// 设定通道(IR:H,L,O)
        /// </summary>
        ...
        public CExtech7440_GPIB(int idNo = 0, string name = "CExtech7400")
        {
            this._idNo = idNo;
            this._name = name;
            this._failStop = "0";
        ...
        private NationalInstruments.NI4882.Device com = null;
        ...
        public bool Open(string comName, out string er, string setting = "9600,n,8,1")
        {
            ...
            try
            {
                ...
                int boardNum = Convert.ToInt16(comName);

                byte primaryAddress = Convert.ToByte(setting);
        ...
        public bool SetTestPara(List<CHPPara.CStep> step, out string er, string proName, bool saveToDev)
        {
            ...
            try
            {
                ...
                if (!WriteCmd(T_fail_stop_off, out er))
                    return false;
                ...
                for (int i = 0; i < step.Count; i++)
                {
                    ...
                    switch (step[i].name)
                    {
                        ...
                        case EStepName.DC:
                            ...
                            //ARC ON/ OFF
                            if (step[i].para[9].setVal.ToString() == "0") cmd = T_arc_off_dcw;
                            else cmd = T_arc_on_dcw;
                            if (!WriteCmd(cmd, out er))
                                return false;
                            System.Threading.Thread.Sleep(110);
        ...
        public bool ReadStatus(out EHPStatus status, out string er)
        {
            ...
            try
            {
                ...
                if (rData.Length > 40) rData.Substring(0, 40);

                if (rData.ToUpper().StartsWith("TEST"))
                {
                    status = EHPStatus.STOPPED;
                    return true;
                }

                string stepName = rData.Substring(0, 4).Trim().ToUpper();
                string stepResult = rData.Substring(4, 12).Trim();
                string stepTimes = rData.Substring(16, 4).Trim();
                string stepNo = rData.Substring(24, 1).Trim();
                string stepValue1 = rData.Substring(26, 5).Trim();
                string stepValue2 = rData.Substring(33, rData.Length - 34);

                if (stepName != "ACW" && stepName != "DCW" && stepName != "IR" && stepName != "GND")
                {
                    if (stepValue2 == ">40") stepName = "ACW";
                }

                string valTemp = string.Empty;
                if (stepValue1.Contains("-"))
                {
                    stepValue1 = "0";
                }
                else
                {
                    for (int j = 0; j < stepValue1.Length; j++)
                    {
                        if (Char.IsDigit(stepValue1[j]) || stepValue1[j] == '.')
                            valTemp += stepValue1[j];
                    }

                    stepValue1 = valTemp;
                    if (stepValue1.EndsWith(".")) stepValue1 += "0";
                }

                valTemp = string.Empty;
                if (stepValue2.Contains("-"))
                {
                    stepValue2 = "0";
                }
                else
                {
                    for (int j = 0; j < stepValue2.Length; j++)
                    {
                        if (Char.IsDigit(stepValue2[j]) || stepValue2[j] == '.')
                            valTemp += stepValue2[j];
                    }

                    stepValue2 = valTemp;
                    if (stepValue2.EndsWith(".")) stepValue2 += "0";
                }

                if (stepResult.ToUpper() == "RAMP" || stepResult.ToUpper() == "DWELL" || stepResult.ToUpper() == "DELAY")
                {
        ...
        public bool ReadResult(int uutMax, int stepMax, out List<CCHResult> uut, out string er)
        {
            ...
            {
                string rData = string.Empty;
                CCHResult result = new CCHResult();
                ...
                if (rData.ToUpper().StartsWith("TEST"))
                {
                    result.Result = rData.ToUpper().Contains("F") ? -1 : 0;
                    uut.Add(result);
                    return true;
                }
                ...
                if (stepResult.ToUpper() == "RAMP" || stepResult.ToUpper() == "DWELL" || stepResult.ToUpper() == "DELAY")
                {
                    result.Result = Convert.ToInt32(stepNo);
                }
                else if (stepResult.ToUpper() == "PASS")
                {
                    result.Result = 0;
                }
                else
                ...
                {
                    case "ACW":
                        result.Step = new List<CStepResult>
                        {
                            new CStepResult
                            {
                                ...
                                Code = stepResult,
                                Name = EStepName.AC,
                                Value = Convert.ToDouble(stepValue1),
                                Unit = "kV"
                            },
                            ...
                            new CStepResult
                            {
                                ...
                                Code = stepResult,
                                Name = EStepName.AC,
                                Value = Convert.ToDouble(stepValue2),
                                Unit = "mA"
                            }
                        };
                        break;
                               ...
                               out List<string> stepUnit, out string er)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < _stepNum; i++)
                {
                    ...
                    string strName = rData.Substring(0, 4).Trim().ToUpper();
                    string strResult = rData.Substring(4, 12).Trim();
                    string strTimes = rData.Substring(16, 4).Trim();
                    string strNo = rData.Substring(24, 1).Trim();
                    string strValue1 = rData.Substring(26, 5).Trim();
                    string strValue2 = rData.Substring(33, rData.Length - 34);

                    if (strName != "ACW" && strName != "DCW" && strName != "IR" && strName != "GND")
                    {
                        if (strValue2 == ">40") strName = "ACW";
                    }
                    ...
                    else
                    {
                        ...
                        {
                            if (Char.IsDigit(strValue1[j]) || strValue1[j] == '.')
                                valTemp += strValue1[j];
                        }

                        strValue1 = valTemp;
                        if (strValue1.EndsWith(".")) strValue1 += "0";
                    }
                    ...
                    else
                    {
                        ...
                        {
                            if (Char.IsDigit(strValue2[j]) || strValue2[j] == '.')
                                valTemp += strValue2[j];
                        }

                        strValue2 = valTemp;
                        if (strValue2.EndsWith(".")) strValue2 += "0";
                    }


                    if (strResult.ToUpper() == "PASS") stepResult.Add(0);
                    else stepResult.Add(1);
                    
                    switch (strName.ToUpper())
                    {
                        case "ACW":
                            stepMode.Add(EStepName.AC);
                            stepVal.Add(Convert.ToDouble(strValue1));
                            stepUnit.Add("kV");
                            stepVal.Add(Convert.ToDouble(strValue2));
                            stepUnit.Add("mA");
                            stepCode.Add(strResult);
                            break;
                        case "DCW":
        ...
        private bool WriteCmd_Inner(string wCmd, out string er, int delayMs = 5, bool readResult = false, int timeOutMs = 500)
        {
            ...
            try
            {
                ...
                if (rData .Length <  5)
                {
                    er = "返回数据错误,发送数据:" + wCmd + ",回读数据为空";
                    return false;
                }

                if(rData == "\u0015")
                {
                    er = "返回数据错误,发送数据:" + wCmd + ",回读数据:15H";
                    return false;
                }
        ...
        private bool SendCmdToHP(string wData, out string rData, out string er, string rEOI = "\n", int wTimeOut = 1000)
        {
            lock (writeLock)
            {
                ...
                try
                {
                    ...
                    {
                        btye = com.ReadByteArray();

                        rData = Encoding.Default.GetString(btye);
