...
namespace GJ.TOOL
{
    public partial class FrmGJI8O14 : Form, IChildMsg
    {
        #region 插件方法
        /// <summary>
        /// 父窗口
        ...
        #region 字段
        private CIO_I8_O14 comMon = null;
        #endregion
        ...
        #region 面板回调函数
        private void FrmGJIO_Load(object sender, EventArgs e)
        {
        ...
        private void btnRead_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!comMon.ReadInSingal(wAddr, out X, out er))
                {
                ...
                labStatus.Text = CLanguage.Lan("读取IO板X1-X8信号成功.");
                labStatus.ForeColor = Color.Blue;
            }
            catch (Exception ex)
            {
        ...
        private void btnOff_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!comMon.CtrlRelayByCmd1(wAddr, 0, out er))
                {
                    labStatus.Text = CLanguage.Lan("设置IO板K1-K14信号OFF失败:") + er;
                    labStatus.ForeColor = Color.Red;
                    return;
                }
                for (int i = 0; i < picK.Length; i++)
                ...
                labStatus.Text = CLanguage.Lan("设置IO板K1-K14信号OFF成功.");
                labStatus.ForeColor = Color.Blue;
            }
            catch (Exception ex)
            {
